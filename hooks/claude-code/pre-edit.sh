...
# Reads the file path from stdin (Claude Code passes JSON) and claims it via lain.
# Always exits 0 — failure must NEVER block Claude Code. Stderr is for diagnostics.
#
# Identity resolution order:
#   1. $LAIN_AGENT_NAME (explicit override)
#   2. Orca env vars (ORCA_PANE_KEY / ORCA_TAB_ID / ORCA_WORKTREE_ID)
#   3. Fallback: "<basename>-<ppid>" (stable for the parent Claude process)
