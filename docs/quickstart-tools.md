...
| `find_anchors` | Orchestration hubs — called by many, calling many, with a real body. Each row names the node's type and path | "Most important." A leaf that calls nothing scores 0 by design. Results are deduped by name keeping the best-scoring definition, so check the path: a ubiquitous method name like `as_str` can still surface, because Calls edges are name-resolved when LSP type info is unavailable and every call site collapses onto one node. |
| `explain_symbol` / `get_anchor_score` / `get_call_sites` by **name** | One node that has that name | "The node you meant." A name defined many times (`parse`, `as_str`, `new`) resolves to one arbitrary definition, and no tool warns that the name was ambiguous. `Calls` edges are name-resolved without LSP type info, so a caller list for a common method name can include unrelated calls — even `str::parse`. Pass a node **id** from `query_graph` instead; ids round-trip between tools and are unambiguous. |
