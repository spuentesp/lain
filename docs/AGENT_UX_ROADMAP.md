...
| 4 | Zero-config MCP startup | 🟡 Mostly done | Implementation-sequence steps 1-9 of 10 landed (`5a4b99c`, `af206f7`, `d5b08e3`, `b214895`, `51446af`, `077f0c9`, `da9acb3`…`9bd8e8e`): mandatory tool classification, the central dispatch gate, phase/progress instrumentation, backgrounded startup re-index, multi-thread runtime, watcher-ready handoff, final overlay reconciliation, a stdio `capabilities_changed` push notification, and per-repository federation readiness gating (`gate_for_dispatch` resolves a tool call's target repo(s) and gates on their own `RepoHealth` instead of the one process-global handle; `get_capabilities` reports both per-repo state and a worst-of aggregate). Missing: step 10 (removing the old blocking-startup helper), a real cooperative cancellation token through the indexing phases, and `spawn_blocking` isolation of the pipeline's synchronous work. |
...
**Status: 🟡 Mostly done.** Steps 1-9 of the 10-step implementation sequence below are landed (`5a4b99c` … `9bd8e8e`) — see "Status" at the top of this document for the full breakdown. Missing: step 10 (removing the old blocking-startup helper), a real cooperative cancellation token through the indexing phases, and `spawn_blocking` isolation of the pipeline's synchronous work.
