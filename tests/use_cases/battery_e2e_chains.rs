...
fn build_fixture() -> (tempfile::TempDir, GraphDatabase) {
    ...
    db.insert_edge(&GraphEdge::new(EdgeType::Calls, cz, orch.clone()))
        .unwrap();
...
fn chain_search_to_anchors_to_blast_to_trace() {
    ...
    let trace_text = trace_dependency(&db, &overlay, "real_hub").expect("trace_dependency step");
    assert!(
        trace_text.contains("helper_a") && trace_text.contains("helper_b"),
    ...
    assert!(
        real_hub_node.name == "real_hub",
        "search step must return real_hub"
    );
}
...
fn chain_dead_to_call_sites_to_explain() {
    ...
    let calls_text = get_call_sites(dir.path(), &db, &overlay, "helper_a").expect("get_call_sites");
    assert!(
        calls_text.contains("real_hub") || calls_text.contains("do_stuff"),
    ...
    let explain_text =
        explain_symbol(dir.path(), &db, &overlay, &occupancy, "real_hub").expect("explain_symbol");
    assert!(
        explain_text.contains("real_hub"),
        "explain chain step: must mention real_hub; got: {explain_text}"
    );
    assert!(
        explain_text.contains("helper_a") || explain_text.contains("helper_b"),
...
fn chain_entry_points_to_master_to_layered() {
    ...
    let entry_text = list_entry_points(&db, &overlay).expect("list_entry_points");
    assert!(
        !entry_text.is_empty(),
    ...
    let master_text = get_master_map(&db, &overlay).expect("get_master_map");
    assert!(
        !master_text.is_empty(),
        "master-map chain step: must return non-empty; got: {master_text}"
    );

    // Step 3: get_layered_map
    let layered_text = get_layered_map(&db, &overlay, 1, "module").expect("get_layered_map");
    assert!(
        !layered_text.is_empty(),
