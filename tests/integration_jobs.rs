...
async fn jobs_persist_and_webhook() {
    ...
    let status_resp = exec
        .call("get_job_status", Some(&status_args))
        .await
        .unwrap();

    let status_val: serde_json::Value =
        serde_json::from_str(&status_resp).expect("parse status response");
    ...
    assert!(
        ...
        "jobs file should include the created job id"
    );
}
...
async fn foreground_job_executes() {
    ...
    assert!(
        resp.contains("Slept for 1 second(s)"),
        "Foreground job should complete immediately"
    );
}
