...
pub fn find_anchors(graph: &GraphDatabase, limit: usize) -> Result<String, LainError> {
    let anchors = graph.find_anchors(limit)?;

    if anchors.is_empty() {
        return Ok("No anchors found".to_string());
    }

    Ok(format!("Top {} anchors:\n{}",
        anchors.len(),
        anchors.iter().enumerate().map(|(i, n)| {
...
pub fn get_anchor_score(graph: &GraphDatabase, symbol: &str) -> Result<String, LainError> {
    let node = graph.get_node(symbol)?;
    match node.and_then(|n| n.anchor_score) {
        Some(s) => Ok(format!("Anchor score for '{}': {:.3}", symbol, s)),
        None => Ok(format!("Symbol '{}' not found or has no anchor score", symbol)),
    }
}

pub fn get_context_depth(graph: &GraphDatabase, symbol: &str) -> Result<String, LainError> {
    let node = graph.get_node(symbol)?
    ...
    match node.depth_from_main {
        ...
        None => Ok(format!("Symbol '{}' has no depth score", symbol)),
    }
}

pub fn find_dead_code(graph: &GraphDatabase) -> Result<String, LainError> {
    ...
    Ok(format!("Found {} dead code symbols:\n{}",
        dead.len(),
        dead.iter().take(20).map(|n| format!("- {}", n.name)).collect::<Vec<_>>().join("\n")
    ))
}

pub fn explain_symbol(graph: &GraphDatabase, symbol: &str) -> Result<String, LainError> {
    let node = graph.get_node(symbol)?
        .ok_or_else(|| LainError::NotFound(format!("Symbol not found: {}", symbol)))?;
    ...
    if !partners.is_empty() {
        lines.push(String::new());
        lines.push("### Frequently Co-Changed With".to_string());
...
pub fn suggest_refactor_targets(graph: &GraphDatabase, limit: usize) -> Result<String, LainError> {
    ...
    if all_nodes.is_empty() {
        return Ok("No nodes found to analyze. Run enrichment first.".to_string());
    }

    // Heuristic: Suggest nodes with high fan-in AND high fan-out (God Objects)
    // OR nodes with high temporal coupling but low anchor score (Fragile/Spaghetti)
    let mut targets: Vec<_> = all_nodes.into_iter().map(|n| {
        ...
        // Score: (fan_in * fan_out) for God Object + (co_change / (anchor + 0.1)) for Spaghetti
    ...
    if targets.is_empty() {
        return Ok("Architecture appears healthy! No high-debt refactor targets identified.".to_string());
    }
