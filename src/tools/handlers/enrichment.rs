...
pub fn run_enrichment(graph: &GraphDatabase, git: &Arc<Mutex<GitSensor>>) -> Result<String, LainError> {
    ...
    tokio::spawn(async move {
        ...
        let (co_change_pairs, latest_commit) = {
            ...
            let pairs = match git_guard.analyze_co_changes(100, 3) {
                Ok(pairs) => pairs,
                Err(e) => {
...
pub fn sync_state(graph: &GraphDatabase, git: &Arc<Mutex<GitSensor>>) -> Result<String, LainError> {
