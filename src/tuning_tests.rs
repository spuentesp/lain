...
fn test_tuning_config_default() {
    ...
    assert_eq!(config.query_prefix, ""); // empty by default — preserves MiniLM behavior
    assert_eq!(config.cross_encoder_top_k, 20); // on by default — visibly better ranking on long ambiguous queries
