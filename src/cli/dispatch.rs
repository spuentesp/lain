...
mod tests {
    ...
    /// against the bare `hook` token is dropped. `schema` is the
    /// `tools/list` dump subcommand (see `cli::schema`). This is
    /// still a regression guard against re-introducing the old
    /// multi-command surface.
    #[test]
