...
pub fn find_untested_functions(
    ...
    let mut result = format!(
        "## Potentially Untested Functions ({} found)\n\n",
        untested.len()
    );
    result.push_str(
        "These functions have no incoming call edges - they may be untested or dead code:\n\n",
    );
...
pub fn get_test_template(
    ...
    template.push_str(&format!(
        "# Test template for {} (auto-generated)\n\n",
        function_name
    ));
...
pub fn find_test_file(_graph: &GraphDatabase, module_path: &str) -> Result<String, LainError> {
    // Strip file extension and common suffixes
    let base_path = module_path.replace(".rs", "").replace("/src/", "/tests/");
...
pub fn get_coverage_summary(
    ...
    let (total_functions, untested_functions) = if let Some(path) = module_path {
        let module_funcs: Vec<_> = all_nodes
            .iter()
    ...
    result.push_str(&format!(
        ...
        structural_reach, entrypoint_coverage,
    ));
...
fn reachable_via_calls(graph: &GraphDatabase, entry_points: &[GraphNode]) -> HashSet<String> {
    ...
    while let Some(id) = queue.pop_front() {
        ...
        for edge in edges {
            ...
            let Ok(Some(target)) = graph.get_node(&edge.target_id) else {
                continue;
            };
