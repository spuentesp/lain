...
use crate::server::tools::handlers::testing::{
    find_test_file, find_untested_functions, get_coverage_summary, get_test_template,
};
...
fn make_test_graph() -> (GraphDatabase, VolatileOverlay) {
    ...
    let untested = GraphNode::new(
        NodeType::Function,
        "unused_fn".to_string(),
        "/src/lib.rs".to_string(),
    );
    // Create a function with incoming calls (tested)
    let tested = GraphNode::new(
        NodeType::Function,
        "tested_fn".to_string(),
        "/src/lib.rs".to_string(),
    );
...
fn test_get_coverage_summary_all() {
    ...
    assert!(
        text.contains("**Structural reach:**"),
        "missing structural reach label: {text}"
    );
    assert!(
        text.contains("**Entrypoint coverage:**"),
        "missing entrypoint coverage label: {text}"
    );
    assert!(
        text.contains("`structural_reach:"),
        "missing machine-readable structural_reach key: {text}"
    );
    assert!(
        text.contains("`entrypoint_coverage:"),
        "missing machine-readable entrypoint_coverage key: {text}"
    );

    // Old headline and verdict are gone.
    assert!(
        !text.contains("Excellent coverage"),
        "stale 'Excellent coverage!' verdict still present"
    );
    assert!(
        !text.contains("Estimated coverage"),
        "stale 'Estimated coverage' headline still present"
    );
    assert!(
        !text.contains("\u{26a0}\u{fe0f} Coverage is below"),
        "stale warning branch still present"
    );
    assert!(
        !text.contains("\u{2139}\u{fe0f} Consider adding tests"),
        "stale info branch still present"
    );
...
fn test_get_coverage_summary_with_entry_point() {
    ...
    assert!(
        result.contains("**Structural reach:**") && result.contains("**Entrypoint coverage:**"),
        "{result}"
    );
    ...
    assert!(
        (0.0..=100.0).contains(&pct_str),
        "out-of-range entrypoint coverage: {pct_str}"
    );
