...
fn co_change_namespace_mismatch_drops_edges() {
    ...
    let production_ns =
        RepoNamespace::from_repo_id(&crate::federation::repo_id::RepoId::new("subject").unwrap());
    graph
        .upsert_node(GraphNode::new_in(
            NodeType::File,
            "core.rs".to_string(),
            core.to_string(),
            &production_ns,
    ...
    graph
        .insert_co_change_edges(&[(core.to_string(), helpers.to_string(), 2)])
        .unwrap();
