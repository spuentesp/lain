...
// On drop, `lsp-bridge`'s `Bridge::drop` walks its tracked
// `LspProcess` children and SIGKILLs them — that's the panic-path
// safety net. Normal shutdown goes through `shutdown` above;
// the `Drop` here is intentionally a no-op marker.
