...
pub(crate) fn parse_depth_range(s: &str) -> Result<std::ops::Range<u32>, String> {
    let (start_s, end_s) = s
        .split_once("..")
...
// The inventory-registered tool wrappers (`register_agent`,
// `claim_files`, every federation/workspace tool, etc.) live in
// `tools_registry.rs`. `dispatch_tool_call` walks that inventory
// first and falls through to the match ladder only for tools that
