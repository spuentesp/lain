...
/// Shared context passed to every inventory-registered MCP tool handler.
/// Holding the deps here lets a tool handler be a `fn` (not a closure
/// with captures) and so participate in `inventory::submit!`.
///
/// Federation, workspace, and core graph tools are still wired
/// through the match arm in `dispatch_tool_call` — the inventory
/// path currently covers presence and server-status / reload. Migrating
/// the rest is a follow-up; the shape is the same.
pub struct McpContext<'a> {
    pub server: Option<&'a LainServer>,
    pub federation: Option<&'a FederatedIndex>,
...
/// is invoked with the shared `McpContext` and the JSON args, and
/// returns `(serialized_text, is_error)`.
///
/// `handler` is a plain `fn` pointer because `inventory::submit!`
/// requires the entry to be expressible at static-init time, and
/// `Box::new(closure)` allocates at runtime. Each tool gets a 4-line
/// wrapper free function that pulls `ctx.server` (or whatever
/// dependency the runner needs), calls the domain runner, and
/// returns the result. Generated by the `declare_presence_tool!` /
...
async fn dispatch_tool_call(
    ...
    // until they're migrated the same way (each one is a one-wrapper
    // + one `inventory::submit!` change; tracked as a follow-up).
    let ctx = McpContext {
        server,
        federation,
        workspaces,
...
// -------------------------------------------------------------------------
// Inventory-registered MCP tool wrappers (Phase 3.2).
//
// Each wrapper exposes a static `fn` that `inventory::submit!` can carry
// at static-init time. The handler closure pattern used by `dispatch_
// presence_tool_outcome` was fine for the match ladder but not for
// inventory collection. `fn` pointers solve that: the inventory payload
// has to be `'static`, and a closure with captures isn't. Each wrapper
// reads from `McpContext` rather than the old `dispatch_presence_tool_
// outcome` server-only signature so a single inventory entry covers
// presence + audit + status + reload tools uniformly.
//
// Federation / workspace tool handlers stay on the match ladder in
// `dispatch_tool_call` for now; migrating them is the same shape
// (one wrapper per tool + one inventory::submit!) and is a follow-up.
// -------------------------------------------------------------------------
